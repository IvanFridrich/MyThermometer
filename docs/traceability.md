...
| FR-11 | LCD row 1: temps; row 2: rotating pages (IP, uptime, status) | `src/main.cpp` renderLcd, `hal/display_target` | Driver impl (Phase 3); row0 I/O alternation + row1 status/WiFi-DN wired in `main.cpp`; HW smoke test | 3/4 |
| FR-12 | Show IP on boot | `src/main.cpp` (logs IP; LCD status) | IP logged on connect; HW smoke test | 4 |
...
| FR-26 | Email on fire + sensor fault, rate-limited 1×/h | `core/email_policy`, `src/main.cpp` checkEmail, `hal/mailer_target` | `test/test_email_policy` (§7, 100% cov); SMTP send (ESP-Mail-Client) + policy wired in `main.cpp` (WDT unsubscribed around the blocking send); HW integration | 5 |
